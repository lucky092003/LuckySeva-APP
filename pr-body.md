...
- **`backend/`** — new FastAPI backend (supabase-py service-role client):
  ...
  - `backend/requirements.txt`, `backend/.env.example`, `backend/.gitignore`
...
- `backend/.env` is gitignored — fill from Supabase dashboard (service_role key + JWT secret)
